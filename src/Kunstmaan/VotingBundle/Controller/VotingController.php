...
class VotingController extends Controller
{
    ...
    public function upVoteAction(Request $request)
    {
        ...
        $response = false;
        $voteForEntity = $this->getDoctrine()->getManager()->getRepository('KunstmaanVotingBundle:UpDown\UpVote')->findBy(array('meta' => $token));
        if (count($voteForEntity) == 0)
        {
            $this->get('event_dispatcher')->dispatch(Events::VOTE_UP, new UpVoteEvent($this->getRequest(), $reference, $value, $token));
            $response = true;
        }
